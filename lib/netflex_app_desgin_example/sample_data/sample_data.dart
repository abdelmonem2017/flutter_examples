import 'package:flutter_examples/netflex_app_desgin_example/models/movie_model.dart';

List<Movie> movies = [
  Movie(
      name: '47 Meters Down',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/47_Meters_Down.jpg',
      posterUrl: 'assets/images/netflex_app_img/47_Meters_Down_banner.jpg'),
  Movie(
      name: 'Annabel Lech',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/Annabellech.jpg',
      posterUrl: 'assets/images/netflex_app_img/Annabellech_banner.jpg'),
  Movie(
      name: 'Avengers Endgame',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/avengersendgame.jpg',
      posterUrl: 'assets/images/netflex_app_img/avengersendgame_banner.jpg'),
  Movie(
      name: 'Gemini man',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/Gemini_man.jpg',
      posterUrl: 'assets/images/netflex_app_img/Gemini_man_banner.jpg'),
  Movie(
      name: 'Joker',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/Joker.jpg',
      posterUrl: 'assets/images/netflex_app_img/Joker_banner.jpg'),
  Movie(
      name: '',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/Malificent.jpg',
      posterUrl: 'assets/images/netflex_app_img/Malificent_banner.jpg'),
  Movie(
      name: 'MIB',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/MIB.jpg',
      posterUrl: 'assets/images/netflex_app_img/MIB_banner.jpg'),
  Movie(
      name: 'moana',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/moana.jpg',
      posterUrl: 'assets/images/netflex_app_img/moana_banner.jpg'),
  Movie(
      name: 'spiderman far from home',
      duration: '90',
      price: '50',
      year: '2015',
      imageUrl: 'assets/images/netflex_app_img/spider_man_far_from_home.jpg',
      posterUrl:
          'assets/images/netflex_app_img/spider_man_far_from_home_banner.jpg')
];
